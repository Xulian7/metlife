...
class BrechasBasicoForm(forms.Form):
    ...
    sexo = forms.ChoiceField(
        required=False,
        label="Género",
        choices=[("", "Seleccione género"), ("hombre", "Hombre"), ("mujer", "Mujer")],
    )
    ...
    ibc_ultimos_10_anios = forms.DecimalField(max_digits=16, decimal_places=2, initial=2000000, label="IBC promedio 10 años")
    anios_cotizados = forms.DecimalField(max_digits=8, decimal_places=2, initial=5, label="Años cotizados")
    anios_por_cotizar = forms.DecimalField(max_digits=8, decimal_places=2, initial=28, label="Años por cotizar")
    meses_cotizados_anio = forms.DecimalField(
        ...
        min_value=0,
        max_value=12,
        label="Meses cotizados por año",
        widget=forms.NumberInput(attrs={"step": "1", "min": "0", "max": "12", "data-projection-control": "true"}),
        help_text="Densidad principal; el detalle también calcula 12, 9, 6 y 0 meses.",
    )
    ...
    tasa_aporte_acumulacion = forms.DecimalField(
        ...
        decimal_places=4,
        initial="0.115",
        label="Tasa acumulación RAIS/ACCAI",
